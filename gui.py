...
from PyQt6.QtCore import (QThread, QObject, pyqtSignal, QUrl, Qt)
...
class LinkDialogue(QDialog):
    ...
    def initUI(self):
        ...
        self.setWindowTitle(('Anki – Download definitions'))
...
class WordDefDialogue(QDialog):
    ...
    def initUI(self):
        ...
        scrollArea.setWidget(scroll_area_content)
        ...
        dialog_buttons = QDialogButtonBox(self)
        dialog_buttons.addButton(QDialogButtonBox.StandardButton.SaveAll)
        dialog_buttons.addButton(QDialogButtonBox.StandardButton.Cancel)
        dialog_buttons.addButton(QDialogButtonBox.StandardButton.Ok)
        dialog_buttons.button(QDialogButtonBox.StandardButton.Ok).clicked.connect(self.create_selected_notes)
        dialog_buttons.button(QDialogButtonBox.StandardButton.Cancel).clicked.connect(self.reject)
        dialog_buttons.button(QDialogButtonBox.StandardButton.SaveAll).clicked.connect(self.save_all)
        ...
        scroll_area_content.adjustSize()  # required to get correct content area size
        scrollArea.verticalScrollBar().adjustSize()  # required to get the correct scrollbar size
        scrollArea.setMinimumWidth(scroll_area_content.size().width() + scrollArea.verticalScrollBar().width())
...
class AddonConfigWindow(QDialog):
    ...
    def initUI(self):
        ...
        h_layout.addWidget(self.editor_cookie, QtCore.Qt.AlignmentFlag.AlignRight)
        layout.addLayout(h_layout, QtCore.Qt.AlignmentFlag.AlignTop)
        ...
        self.cb_pronunciation_uk.setChecked(
            self.config['pronunciation_uk'] if 'pronunciation_uk' in self.config else True)
        h_layout.addWidget(self.cb_pronunciation_uk, QtCore.Qt.AlignmentFlag.AlignRight)
        layout.addLayout(h_layout, QtCore.Qt.AlignmentFlag.AlignTop)
        ...
        self.cb_pronunciation_us.setChecked(
            self.config['pronunciation_us'] if 'pronunciation_us' in self.config else True)
        h_layout.addWidget(self.cb_pronunciation_us, QtCore.Qt.AlignmentFlag.AlignRight)
        layout.addLayout(h_layout, QtCore.Qt.AlignmentFlag.AlignTop)
        ...
        btn_Ok = QPushButton()
        btn_Ok.setText('Ok')
        btn_bottom_layout.addWidget(btn_Ok, QtCore.Qt.AlignmentFlag.AlignRight)
        btn_Cancel = QPushButton()
        btn_Cancel.setText('Cancel')
        btn_bottom_layout.addWidget(btn_Cancel, QtCore.Qt.AlignmentFlag.AlignRight)
...
class WParseSavedWL(QObject):
    ...
    def onEvent(self, evt, *args):
        ...
        if evt == 'need_to_stop':
            ...
            tooltip(
                (str('Stopping all error')),
                parent=mw,
            )
        if evt == 'error':
            self.need_to_stop = True
            showText(("Fetching failed:\n%s") % self._rewriteError(args[0]))
    ...
    def on_add_word(self, word_entry):
        ...
        self.fetched += 1
        tooltip(
            (str(self.fetched) + ' / ' + str(self.word_to_fetch)),
            parent=mw,
        )
...
Please upgrade to the latest version of Anki."""
        ...
        # error message
            ...
            return ("Cambridge Dictionary is under maintenance. Please try again in a few minutes.")
